...
public:
    ...
    cuckoohash_map(size_t n = DEFAULT_SIZE,
        ...
        : num_inserts_(), num_deletes_(), hash_fn(hf), eq_fn(eql) {
    ...
    bool insert(BOOST_FWD_REF(K) key, BOOST_FWD_REF(V) val) {
        return cuckoo_insert_loop(hashed_key(key), boost::forward<K>(key),
                                  boost::forward<V>(val));
    }
    ...
    bool update(const K& key, BOOST_FWD_REF(V) val) {
        ...
        const cuckoo_status st =
            cuckoo_update(hv, b.i[0], b.i[1], key, boost::forward<V>(val));
        return (st == ok);
    }
    ...
    typename boost::enable_if_c<
        boost::is_convertible<Updater, updater_type>::value, bool>::type
    update_fn(const K& key, Updater fn) {
        const size_t hv = hashed_key(key);
        const TwoBuckets b = snapshot_and_lock_two(hv);
...
private:
    ...
    // hashpower.
    TwoBuckets snapshot_and_lock_two(const size_t hv) const
    ...
    // for use afterwards.
    template <typename K, typename V>
    void add_to_bucket(const partial_t partial, Bucket& b, const size_t slot,
                       BOOST_FWD_REF(K) key, BOOST_FWD_REF(V) val) {
        assert(!b.occupied(slot));
        b.setKV(slot, partial, boost::forward<K>(key), boost::forward<V>(val));
    ...
    cuckoo_status cuckoo_insert(const size_t hv, TwoBuckets b,
        ...
        if (res1 != -1) {
            add_to_bucket(partial, b0, res1, boost::forward<K>(key),
                          boost::forward<V>(val));
            return ok;
        }
        if (res2 != -1) {
            add_to_bucket(partial, b1, res2, boost::forward<K>(key),
                          boost::forward<V>(val));
            return ok;
        }
     ...
     */
    template <typename K, typename V>
    bool cuckoo_insert_loop(size_t hv, BOOST_FWD_REF(K) key,
                            BOOST_FWD_REF(V) val) {
        cuckoo_status st;
        do {
            TwoBuckets b = snapshot_and_lock_two(hv);
...
private:
    ...
    cacheint *num_inserts_, *num_deletes_;
